...
class Request
{
    ...
    protected function _getRequestBody()
    {
        ...
        if ($this->_signedPost) {
            $this->_posts = Signatures::signData($this->_posts, $this->_excludeSigned, $this->_parent->getPlatform(), $this->_parent->getAuthKey());
        }
    ...
    protected function _buildHttpRequest()
    {
        ...
        if (strncmp($endpoint, 'http:', 5) !== 0 && strncmp($endpoint, 'https:', 6) !== 0) {
            if ($this->_apiVersion === 999) {
                $endpoint = Constants::API_URLS[$this->_apiVersion][mt_rand(0, 2)].$endpoint;
            } else {
        ...
        if ($this->_signedGet) {
            $this->_params = Signatures::signData($this->_params, [], $this->_parent->getPlatform(), $this->_parent->getAuthKey());
        }
