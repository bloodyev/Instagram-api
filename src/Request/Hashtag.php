...
class Hashtag extends RequestCollection
{
    ...
    /**
     * Mark TagsStoryResponse story media items as seen.
     *
     * The "story" property of a `TagsStoryResponse` only gives you a list of
        ...
        array $items)
    {
        ...
        $sourceId = '';
        if ($hashtagFeed->getStory() instanceof Response\Model\Reel) {
            $sourceId = $hashtagFeed->getStory()->getId();
        }
        if (!strlen($sourceId)) {
            throw new \InvalidArgumentException('Your provided TagsStoryResponse is invalid and does not contain any Hashtag Story-Tray ID.');
        }
        ...
        return $this->ig->internal->markStoryMediaSeen($items, $sourceId, 'hashtag_feed');
    }
}
